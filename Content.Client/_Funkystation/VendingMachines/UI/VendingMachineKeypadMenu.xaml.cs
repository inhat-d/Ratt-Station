...
public enum VendingMachineCodeResult
{
    Success,
    NoAccess,
    LowFunds,
...
public sealed partial class VendingMachineKeypadMenu : FancyWindow
{
    ...
    public event Func<int, VendingMachineCodeResult>? OnCodeEntered;
    ...
    private readonly string[] _successPhrases = [
        "vending-machine-keypad-success-enjoy",
        "vending-machine-keypad-success-thx",
        "vending-machine-keypad-success-yum",
        "vending-machine-keypad-success-vend"
    ];
    ...
    public event Action<VendingMachineWithdrawMessage>? OnWithdraw;
    ...
    public void Populate(List<VendingMachineInventoryEntry> inventory, bool enabled)
    {
        ...
        for (var i = 0; i < inventory.Count; i++)
        {
            ...
            var code = GetCode(i);
            var price = (int)(entry.Price * _priceMultiplier);

            var slot = new VendingMachineGridSlot();
            slot.SetItem(entry.ID, code, entry.Amount, price, entry.Amount == 0);
    ...
    public void SetCredits(int credits, double priceMultiplier)
    {
        Credits = credits;
    ...
    private void UpdateCreditsLabel()
    {
        CreditsLabel.Text = Credits > 0
            ? Loc.GetString("vending-machine-keypad-credits", ("credits", Credits))
            : Loc.GetString("vending-machine-keypad-insert-credits");
    }
    ...
    private void SubmitBuffer()
    {
        ...
        {
            ShowFeedback(Loc.GetString("vending-machine-keypad-error"), false, true);
            return;
        }
        ...
        if (slot.SoldOut)
        {
            ShowFeedback(Loc.GetString("vending-machine-keypad-sold"), false, true);
            return;
        }

        var result = OnCodeEntered?.Invoke(index) ?? VendingMachineCodeResult.Success;

        switch (result)
        {
            case VendingMachineCodeResult.NoAccess:
                ShowFeedback(Loc.GetString("vending-machine-keypad-denied"), false, true);
                return;
            case VendingMachineCodeResult.LowFunds:
                ShowFeedback(Loc.GetString("vending-machine-keypad-low-funds"), false, true);
                _chuteRattleDelay = 0.55f;
                _chuteRattleTimer = 0.35f;
                return;
            case VendingMachineCodeResult.Success:
                ShowFeedback(Loc.GetString(_random.Pick(_successPhrases)), true, true);
                _chuteRattleDelay = 0.55f;
                _chuteRattleTimer = 0.35f;
                return;
            default:
    ...
    private void ShowFeedback(string text, bool success, bool playSound)
    {
        _bufferLetter = null;
        _bufferNumber = null;
