...
namespace Content.Client._Pirate.BarbellBench;
...
public sealed class BarbellBenchVisualizerSystem : VisualizerSystem<BarbellBenchVisualsComponent>
{
    ...
    protected override void OnAppearanceChange(EntityUid uid, BarbellBenchVisualsComponent component, ref AppearanceChangeEvent args)
    {
        ...
        var hasItemSlots = TryComp<ItemSlotsComponent>(uid, out _);
        EntityUid? attachedBarbell = null;
        if (hasItemSlots && _container.TryGetContainer(uid, slotId, out var slotContainer) && slotContainer.Count > 0)
        ...
        if (hasItemSlots && SpriteSystem.LayerMapTryGet((uid, args.Sprite), slotId, out var attachedLayer, false))
        {
