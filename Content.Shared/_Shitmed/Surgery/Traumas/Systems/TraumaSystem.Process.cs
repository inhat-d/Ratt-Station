    ...
    private void OnApplyTrauma(Entity<WoundableComponent> ent, ref ApplyTraumaEvent args)
    {
        ...
        else if (args.TraumaType == Dismemberment)
        {
            if (_wound.IsWoundableRoot(target)
                || !_dismembermentInProgress.Add(targetChosen))
            {
                args.Handled = true;
                return;
            }
            ...
            try
            {
                if (_wound.TryInduceWound(targetChosen, "Blunt", 0f, out var woundInduced)) // We need this to add the trauma into.
                {
                    AddTrauma(
                        targetChosen,
            ...
            {
                _dismembermentInProgress.Remove(targetChosen);
            }
