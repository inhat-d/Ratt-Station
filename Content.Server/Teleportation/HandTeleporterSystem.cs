...
public sealed class HandTeleporterSystem : EntitySystem
{
    ...
    private void HandlePortalUpdating(EntityUid uid, HandTeleporterComponent component, EntityUid user)
    {
        ...
        if (Deleted(component.FirstPortal) && Deleted(component.SecondPortal))
        {
            ...
            _adminLogger.Add(LogType.EntitySpawn, LogImpact.High, $"{ToPrettyString(user):player} відкрив {ToPrettyString(component.FirstPortal.Value)} в {Transform(component.FirstPortal.Value).Coordinates} за допомогою {ToPrettyString(uid)}");
    ...
    private void FizzlePortals(EntityUid uid, HandTeleporterComponent component, EntityUid user, bool instability)
    {
        ...
        if (portalStrings != "")
            _adminLogger.Add(LogType.EntityDelete, LogImpact.High, $"{ToPrettyString(user):player} закрив {portalStrings} за допомогою {ToPrettyString(uid)}");
